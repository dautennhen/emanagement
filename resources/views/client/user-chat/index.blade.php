                            ...
                            <li id="dp_{{$userChat->id}}">
                                <a href="javascript:void(0)" id="dpa_{{$userChat->id}}"
                                    ...
                                    <span @if($userChat->message_seen == 'no' && $userChat->user_one != $userChat->id) class="font-bold" @endif> {{$userChat->name}}
                                        <small class="text-simple"> @if($userChat->last_message){{  ucwords(\Carbon\Carbon::parse($userChat->last_message)->diffForHumans())}} @endif
                                            @if(\App\User::isAdmin($userChat->id))
                                                <label class="btn btn-danger btn-xs btn-outline">{{ ucwords(__('app.admin')) }}</label>
                                            @elseif(\App\User::isClient($userChat->id))
                                                <label class="btn btn-success btn-xs btn-outline">{{ ucwords(__('app.clients')) }}</label>
                                            @else
                                                <label class="btn btn-warning btn-xs btn-outline">{{ ucwords(__('app.employee')) }}</label>
                                            @endif
                                        </small>
                                    </span>
                                </a>
...
<script type="text/javascript">
    ...
    function deleteMessage(messageId){
        ...
        swal({
            ...
            text: "{{ mb_convert_case(__('app.ask.Youwillnotbeabletorecoverthedeletedmessage'),MB_CASE_TITLE,"UTF-8") }}",
