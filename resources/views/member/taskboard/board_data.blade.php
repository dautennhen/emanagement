<h4>@lang('app.menu.tasks') @lang('app.from') <span id="startDate">{{ $startDate }}</span> <i class="ti-arrow-right"></i> <span id="endDate">{{ $endDate }}</span></h4>
    ...
    <div class="panel col-xs-3 board-column p-0" data-column-id="{{ $column->id }}" >
        <div class="panel-heading p-t-5 p-b-5"  >
            <div class="panel-title">
                <h6 style="color: {{ $column->label_color }}">
                    {{-- {{ __('app.'.lcfirst($column->column_name)) }} --}}
                    @if($column->column_name=='Complete'||$column->column_name=='Incomplete'||$column->column_name=='Completed')
                    {{ __('app.'.lcfirst($column->column_name)) }}
                    @else 
                        {{ lcfirst($column->column_name) }}
                    @endif
        ...
        <div class="panel-body" id="taskBox_{{ $column->id }}" style="height: 70vh; overflow-y: auto">
            <div class="row">
                <div class="col-xs-12" data-status="{{ $column->slug }}" style="height: 400px !important;">
                            ...
                            <div class="panel-body">
                                ...
                                <div class="bg-grey p-10">
                                    ...
                                    @elseif($task->due_date->setTimezone($global->timezone)->isToday())
                                        <span class="text-success"><i class="icon-calender"></i> {{ ucwords(__('app.today')) }}</span>
                                    @else
...
//                //console.log(el, $(el).index());
            ...
            $.easyAjax({
                ...
                type: 'POST',
                async: false,
                data:{status:status, boardColumnIds: boardColumnIds, taskIds: taskIds, prioritys: prioritys,'_token':'{{ csrf_token() }}', draggingTaskId: draggingTaskId, draggedTaskId: draggedTaskId},
