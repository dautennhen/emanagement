                ...
                <div class="vtabs customvtab m-t-10">
                    ...
                    <div class="tab-content">
                        <div id="vhome3" class="tab-pane active">
                            <div class="row">
                                <div class="col-sm-12">
                                    <h3 class="box-title m-b-0">{{ mb_convert_case(__('app.Righttobeinformed'),MB_CASE_TITLE,"UTF-8") }}</h3>
                                    <div class="row b-t m-t-20 p-10">
                                        <div class="col-md-12">
                                            {!! Form::open(['id'=>'editSettings','class'=>'ajax-form','method'=>'POST']) !!}
                                            <label for="">{{ mb_convert_case(__('app.EnableTerms&Conditionscustomersfooter'),MB_CASE_TITLE,"UTF-8") }}</label>
                                            <div class="form-group">
                                                <label class="radio-inline">
                                                    <input type="radio"
                                                           class="checkbox"
                                                           @if($gdprSetting->terms_customer_footer) checked @endif
                                                           value="1" name="terms_customer_footer">{{ ucwords(__('app.yes')) }}
                                                </label>
                                                <label class="radio-inline m-l-10">
                                                    <input type="radio"
                                                           @if($gdprSetting->terms_customer_footer==0) checked @endif
                                                           value="0" name="terms_customer_footer">{{ ucwords(__('app.no')) }}
                                                </label>


                                            </div>
                                            <hr>
                                            <label for="">{{ mb_convert_case(__('app.Termsandcondition'),MB_CASE_TITLE,"UTF-8") }}</label>
                                            ...
                                            <hr>
                                            <label for="">{{ mb_convert_case(__('app.Privacyandpolicy'),MB_CASE_TITLE,"UTF-8") }}</label>
