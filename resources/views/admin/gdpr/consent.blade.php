                ...
                <div class="vtabs customvtab m-t-10">
                    ...
                    <div class="tab-content">
                        <div id="vhome3" class="tab-pane active">
                            <div class="row">
                                <div class="col-sm-12">
                                    <h3 class="box-title m-b-0">{{ mb_convert_case(__('app.Consent'),MB_CASE_TITLE,"UTF-8") }}</h3>
                                    <div class="row b-t m-t-20 p-10">
                                        <div class="col-md-12">
                                            {!! Form::open(['id'=>'editSettings','class'=>'ajax-form','method'=>'POST']) !!}
                                            <label for="">{{ mb_convert_case(__('app.Enableconsentforcustomers'),MB_CASE_TITLE,"UTF-8") }}</label>
                                            <div class="form-group">
                                                <label class="radio-inline">
                                                    <input type="radio"
                                                           class="checkbox"
                                                           @if($gdprSetting->consent_customer) checked @endif
                                                           value="1" name="consent_customer">{{ucwords(__('app.yes'))}}
                                                </label>
                                                <label class="radio-inline m-l-10">
                                                    <input type="radio"
                                                           @if($gdprSetting->consent_customer==0) checked @endif
                                                           value="0" name="consent_customer">{{ ucwords(__('app.no')) }}
                                                </label>


                                            </div>
                                            <hr>
                                            <label for="">{{ mb_convert_case(__('app.EnableconsentforLeads'),MB_CASE_TITLE,"UTF-8") }}</label>
                                            <div class="form-group">
                                                <label class="radio-inline">
                                                    <input type="radio"
                                                           class="checkbox"
                                                           @if($gdprSetting->consent_leads) checked @endif
                                                           value="1" name="consent_leads">{{ucwords(__('app.yes'))}}
                                                </label>
                                                <label class="radio-inline m-l-10">
                                                    <input type="radio"
                                                           @if($gdprSetting->consent_leads==0) checked @endif
                                                           value="0" name="consent_leads">{{ ucwords(__('app.no')) }}
                                                </label>
                                            ...
                                            <hr>
                                            <label for="">{{ mb_convert_case(__('app.Publicpageconsentinformationblock'),MB_CASE_TITLE,"UTF-8") }}</label>
                                            <div class="form-group">
                                            ...
                                            <button type="button" onclick="submitForm();" class="btn btn-primary">
                                                {{ucwords(__('app.submit'))}}
                                            </button>
                                            {!! Form::close() !!}

                                            <hr>
                                            <h3 class="box-title m-b-0">{{ mb_convert_case(__('app.Purposeofconsent'),MB_CASE_TITLE,"UTF-8") }}</h3>
                                            <p>
                                                <button type="button" id="addConsent" class="btn btn-success"><i
                                                            class="icon-plus"></i> {{ mb_convert_case(__('app.Addnewconsent'),MB_CASE_TITLE,"UTF-8") }}
                                                </button>
                                            </p>
    ...
    <script>
        ...
        $('body').on('click', '.sa-params', function(){
            ...
            swal({
                ...
                text: "{{ mb_convert_case(__('app.ask.Youwillnotbeabletorecoverthedeletednotice'),MB_CASE_TITLE,"UTF-8") }}",
                type: "warning",
