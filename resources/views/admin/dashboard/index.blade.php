                            ...
                            <ul class="list-task list-group" data-role="tasklist">
                                ...
                                @forelse($newTickets as $key=>$newTicket)
                                    <li class="list-group-item" data-role="task">
                                        {{ ($key+1) }}. <a href="{{ route('admin.tickets.edit', $newTicket->id) }}" class="text-danger"> {{  ucfirst($newTicket->subject) }}</a> <i>{{ucwords(trans($newTicket->created_at->diffForHumans()))}}</i>
                                    </li>
                                @empty
                                    <li class="list-group-item" data-role="task">
...
<script>
    ...
    $(document).on('click', '.dashboard-settings', function (e) {
        e.stopPropagation();
    });
</script>
@endpush
