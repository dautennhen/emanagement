        ...
        <div class="col-md-6">
            <div class="white-box">
                ...
                <h3 class="box-title m-b-0">{{ $contract->subject }}
                    <a href="{{ route('admin.contracts.show', md5($contract->id)) }}" target="_blank" class="btn btn-sm btn-default pull-right">@lang('modules.module.ViewContract')</a>
                </h3>
                                ...
                                <textarea name="contract_detail" id="contract_detail"
                                    ...
                                    @if($contract->signature)
                                        <div class="text-right" id="signature-box">
                                            <h2 class="box-title">@lang('modules.estimates.signature') (@lang('modules.gdpr.customers'))</h2>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input class="form-control number" id="amount" name="amount" value="{{ $contract->amount ?? '' }}">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <div>
                                <select class="select2 form-control" data-placeholder="@lang('app.client')" id="contractType" name="contract_type">
                                    @foreach($contractType as $type)
                                        <option
                                                @if($type->id==$contract->contract_type_id) selected
                                                @endif
...
@push('footer-script')
    ...
    <script>
        ...
        $(document).ready(function () {
          //Get
            var bla = $('input.number').val();
            bla=bla
            .replace(/\D/g, "")
            .replace(/\B(?=(\d{3})+(?!\d))/g, ",")
            ;
            //Set
            $('input.number').val(bla);
        });
