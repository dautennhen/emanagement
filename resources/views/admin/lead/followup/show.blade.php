...
@section('page-title')
    <div class="row bg-title">
        ...
        <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
            <h4 class="page-title"><i class="{{ $pageIcon }}"></i> {{ ucwords(__($pageTitle)) }} #{{ $lead->id }} - <span class="font-bold">{{ ucwords($lead->company_name) }}</span></h4>
        </div>
        <!-- /.page title -->
        <!-- .breadcrumb -->
                            ...
                            <div class="row">
                                <div class="col-md-12" id="follow-list-panel">
                                    <div class="white-box">
                                        ...
                                        <div class="row m-b-10">
                                            <div class="col-md-5">
                                                ...
                                                <a href="javascript:;" id="show-new-follow-panel"
                                                   class="btn btn-success btn-outline"><i class="fa fa-plus"></i> {{ ucwords(__('modules.followup.newFollowUp')) }}</a>
                                                @endif
                                            </div>
                                            <div class="col-md-3">
                                            ...
                                            <div class="col-md-4">
                                                <select class="selectpicker" data-style="form-control" id="sort-task" data-lead-id="{{ $lead->id }}">
                                                    <option value="id">{{ ucwords(__('modules.tasks.lastCreated')) }}</option>
                                                    <option value="next_follow_up_date">{{ ucwords(__('modules.tasks.dueSoonASC')) }}</option>
                                                    <option value="desc">{{ ucwords(__('modules.tasks.dueSoonDESC')) }}</option>
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="list-group" id="listGroup" style="max-height: 400px; overflow-y: auto;">
                                            @forelse($lead->follow as $follow)
                                                <a href="javascript:;" data-follow-id="{{ $follow->id }}" id="followUpBox{{ $follow->id }}" class="list-group-item edit-task">
                                                    <h4 class="list-group-item-heading sbold"> @lang('app.next_follow_up'):  {{ $follow->next_follow_up_date->format($global->date_format) }}<button type="button" title="@lang('app.delete')" onclick="removeFollow({{ $follow->id }});" class="btn btn-danger float-right followup-remove"><i class="fa fa-remove"></i></button></h4>
                                                    ...
                                                    <div class="row margin-top-5">
                                                        ...
                                                        <div class="col-md-6">
                                                            @lang('app.createdOn'):
                                                            {{ $follow->created_at->format($global->date_format) }}
                                        ...
                                        <span class="help-block"><b>{{ ucwords(__('app.contracts.note')) }}: </b>  @lang('modules.followup.followUpNote')</span>
                                    </div>
                                </div>
