    ...
    <div class="panel col-xs-3 board-column p-0" data-column-id="{{ $column->id }}" >
        <div class="panel-heading p-t-5 p-b-5" >
            <div class="panel-title">
                {{-- @if($column->type=="converted")
                    <input type="text"  value="{{$column->type}}" id="gtr">
                    <input type="text"  value="{{$column->id}}" id="id_gtr">
                @endif --}}
                
                <h6 style="color: {{ $column->label_color }}">@lang($column->type)
            ...
            <div class="row">
                <div class="col-xs-12" style="height: 400px !important;" >
                    @foreach($column->leads as $lead)
                        <div class="panel panel-default <?php if($i != count($boardColumns)) echo 'lobipanel'; ?> view-task" data-task-id="{{ $lead->id }}">
                            <div class="panel-body">
                                <div class="p-10 font-12 font-semi-bold">{{ ucfirst($lead->client_name) }}</div>
...
<script>
    $(function () {
        ...
        var data ='@lang("app.menu.lead") @lang("app.from")<strong> {{ \Carbon\Carbon::parse($startDate)->format($global->date_format) }} </strong> to <strong>{{ \Carbon\Carbon::parse($endDate)->format($global->date_format) }}</strong>';
        ...
        // $(document).ready(function(lobiPanel) {
        //     //console.log(document.getElementById("gtr").value);
            
        //     var id_gtr=document.getElementById("id_gtr").value;
        //     var task="#taskBox_"+id_gtr;
        //     // document.getElementById(task).setAttribute("class", "democlass");           
        //     document.getElementsByClassName('.view-task').setAttribute("data-sortable", "false");  
        ...
        $('.lobipanel').on('dragged.lobiPanel', function (ev, lobiPanel) {
            ...
            $.easyAjax({
                ...
                success: function (response) {
                    ...
                    // //console.log($me.data("sortable"))
                    // $me.data("sortable",false)
                    // //console.log($me.data("sortable"))
            
                    //     $me.removeAttr("data-sortable");
                    //     $me.lobiPanel({ sortable: false});
                }
            });
        ...
        // not allow drop in last panel
        $('.board-column').last().find('.lobipanel-parent-sortable').removeClass('lobipanel-parent-sortable')
