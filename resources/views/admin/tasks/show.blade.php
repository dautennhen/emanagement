...
<div class="r-panel-body">
    ...
    <div class="row">
        <div class="row">
            <div class="col-xs-12">
                <a href="{{route('admin.all-tasks.edit',$task->id)}}" class="btn btn-info btn-sm m-b-10 btn-rounded btn-outline pull-right m-l-5"> <i class="fa fa-edit"></i> {{ mb_convert_case(__('app.edit'),MB_CASE_TITLE,"UTF-8") }}</a>
                ...
                <a href="javascript:;" id="reminderButton" class="btn btn-info btn-sm m-b-10 btn-rounded btn-outline pull-right @if($task->board_column->slug == 'completed') hidden @endif" title="@lang('messages.remindToAssignedEmployee')"><i class="fa fa-envelope"></i> {{ mb_convert_case(__('modules.tasks.reminder'),MB_CASE_TITLE,"UTF-8") }}</a>
            ...
            <div class="col-xs-12">
                <h5>{{ ucwords($task->heading) }}
                    ...
                    @if($task->priority == 'high')
                        ...
                        <span class="text-dark">@lang('modules.tasks.priority') ></span>  {{ mb_convert_case(__('app.'.$task->priority),MB_CASE_UPPER,"UTF-8") }}
                    </label>
                                ...
                                <div class="col-xs-6 col-md-12 font-12 m-t-10 m-b-10">
                                    ...
                                    <span>
                                        @foreach($task->label as $key => $label)
                                            <label class="badge font-semi-bold" style="background:{{ $label->label->label_color }}">{{ ucwords($label->label->label_name) }} </label>
                                        @endforeach
                                    </span>
                                </div>
                            @endif
