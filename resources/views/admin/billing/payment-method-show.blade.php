                ...
                <div class="col-12 col-sm-12 mt-40 text-center" id="onlineBox">
                    @if(($stripeSettings->show_pay))
                        <div class="form-group payment-type align-items-center justify-content-center d-flex">
                            ...
                            @if($stripeSettings->vnp_TmnCode != null && $stripeSettings->vnp_HashSecret != null  && $stripeSettings->vnpay_status == 'active')
                            
                            <form id="paystack-form" action="{{ route('admin.payments.vnpay') }}" class="paystack-form d-webkit-inline-box" method="POST">
                                <input type="hidden" id="name" name="name" value="{{ $user->name }}">
                                <input type="hidden" id="paystackEmail" name="vnpayEmail" value="{{ $user->email }}">
                                <input type="hidden" name="plan_id" value="{{ $package->id }}">
                                <input type="hidden" name="type" value="{{ $type }}">
                                {{ csrf_field() }}
                                <button type="submit" id="card-button" class="btn btn-success">
                                    <img height="15px" id="company-logo-img" src="https://s3-eu-west-1.amazonaws.com/pstk-integration-logos/paystack.jpg"> VNpay
                                </button>
                            </form>
                            @endif
