    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->editColumn('status', function ($row) {
                $status = '';
                if ($row->status == 'waiting') {
                    $status .= '<label class="label label-warning">' . ucwords(__("modules.estimates.$row->status")) . '</label>';
                } else if ($row->status == 'draft') {
                    $status .= '<label class="label label-primary">' . ucwords(__("modules.estimates.$row->status")) . '</label>';
                } else if ($row->status == 'declined') {
                    $status .= '<label class="label label-danger">' . ucwords(__("modules.estimates.$row->status")) . '</label>';
                } else {
                    $status .= '<label class="label label-success">' . ucwords(__("modules.estimates.$row->status")) . '</label>';
                }

                if (!$row->send_status && $row->status != 'draft') {
                    $status .= '<br><br><label class="label label-inverse">' . ucwords(__("modules.estimates.$row->status")) . '</label>';
                }
                return $status;
            })
