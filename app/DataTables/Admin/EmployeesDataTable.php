...
class EmployeesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->addColumn('role', function ($row) use ($roles, $firstAdmin) {
                ...
                //dd(user());
                if ($row->id != user()->id ) {
                    $user_role = $this->getUserRole($row->id);
                    ...
                    $status .= '<ul role="menu" class="dropdown-menu pull-right" id="sizelist">';
                    foreach ($roles as $role) {
                        $selected = in_array($role->id,$user_role) ? 'active':'';//($user_role==$role->id)?'active':'';
                        if ($role->id <= 3) {
                            $status .= '<li class="'. $selected .'"><a href="javascript:;" data-user-id="' . $row->id . '" class="assign_role" data-role-id="' . $role->id . '">' . __($role->name) . '</a></li>';
                        } else {
                            $status .= '<li  class="'. $selected .'"><a href="javascript:;" data-user-id="' . $row->id . '" class="assign_role" data-role-id="' . $role->id . '">' .__($role->name) . '</a></li>';
                        }
                    }
    ...
    function getUserRole($userId){
        ...
        return $arr;
        //return User::find($userId)->role();//->toArray();
    }
    /**
