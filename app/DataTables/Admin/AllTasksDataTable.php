...
class AllTasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->editColumn('board_column', function ($row) use ($taskBoardColumns) {
                ...
                if($row->board_column=='Complete'||$row->board_column=='Incomplete'||$row->board_column=='Completed'){
                    $status .= '<button aria-expanded="true" data-toggle="dropdown" class="btn dropdown-toggle waves-effect waves-light btn-xs"  style="border-color: ' . $row->label_color . '; color: ' . $row->label_color . '" type="button">' . mb_convert_case(__("app.$row->board_column"), MB_CASE_TITLE,"UTF-8") . ' <span class="caret"></span></button>';
                }else
                ...
                foreach ($taskBoardColumns as $key => $value) {
                    if($value->column_name=='Complete'||$value->column_name=='Incomplete'||$value->column_name=='Completed')
                    {
                        $status .= '<li><a href="javascript:;" data-task-id="' . $row->id . '" class="change-status" data-status="' . $value->slug . '">' . __("app.$value->column_name") . '  <span style="width: 15px; height: 15px; border-color: ' . $value->label_color . '; background: ' . $value->label_color . '"
    ...
    public function query(Task $model)
    {
        ...
        $model = $model->leftJoin('projects', 'projects.id', '=', 'tasks.project_id')
            ...
            ->select('tasks.id', 'projects.project_name', 'tasks.heading', 'client.name as clientName', 'creator_user.name as created_by', 'creator_user.image as created_image', 'tasks.due_date', 'taskboard_columns.column_name as board_column', 'taskboard_columns.label_color', 'tasks.project_id', 'tasks.is_private')
    ...
    protected function getColumns()
    {
        ...
        $dsData =  [
            ...
            __('app.columnStatus') => ['data' => 'board_column', 'name' => 'taskboard_columns.board_column', 'exportable' => false, 'searchable' => false],
