            ...
            ->editColumn('status', function ($row) {
                ...
                if ($row->credit_note) {
                    ...
                    if ($row->status == 'unpaid') {
                        $status .= '<label class="label label-danger">' . mb_convert_case(__('app.' . $row->status), MB_CASE_UPPER, "UTF-8") . '</label>';
                    } elseif ($row->status == 'paid') {
                        $status .= '<label class="label label-success">' . mb_convert_case(__('app.' . $row->status), MB_CASE_UPPER, "UTF-8") . '</label>';
                    } elseif ($row->status == 'draft') {
                        $status .= '<label class="label label-primary">' . mb_convert_case(__('app.' . $row->status), MB_CASE_UPPER, "UTF-8") . '</label>';
                    } elseif ($row->status == 'canceled') {
                        $status .= '<label class="label label-danger">' . mb_convert_case(__('app.' . $row->status), MB_CASE_UPPER, "UTF-8") . '</label>';
                    } elseif ($row->status == 'review') {
                        return '<label class="label label-warning">' . mb_convert_case(__('app.' . $row->status), MB_CASE_UPPER, "UTF-8") . '</label>';
                    } else {
                        $status .= '<label class="label label-info">' . mb_convert_case(__('modules.invoices.partial'), MB_CASE_UPPER, "UTF-8") . '</label>';
                    }
                }
                if (!$row->send_status && $row->status != 'draft') {
                    $status .= '<br><br><label class="label label-inverse">' . mb_convert_case(__('modules.invoices.notSent'), MB_CASE_UPPER, "UTF-8") . '</label>';
                }
                return $status;
            })
