...
class LeadsDataTable extends BaseDataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                if ($row->client_id == null || $row->client_id == '') {
                    $follow = '<li><a href="' . route('admin.clients.create') . '/' . $row->id . '"><i class="fa fa-user"></i> ' . ucwords(__('modules.lead.changeToClient')) . '</a></li>';
                    if ($row->next_follow_up == 'yes') {
                        $follow .= '<li onclick="followUp(' . $row->id . ')"><a href="javascript:;"><i class="fa fa-thumbs-up"></i> ' . ucwords(__('modules.lead.addFollowUp')) . '</a></li>';
                    }
                } else {
                    $follow = '';
                }
                ...
                <ul role="menu" class="dropdown-menu pull-right">
                    ...
                    <li><a href="' . route('admin.leads.edit', $row->id) . '"><i class="fa fa-edit"></i> ' . ucwords(__('modules.lead.edit')) . '</a></li>
            ...
            ->addColumn('status', function ($row) {
                ...
                foreach ($status as $st) {
                    ...
                    // $st->type = ''.ucwords(__("app.$st->type")).'';
                    $st->type = trans($st->type);
                    if ($selected !='' && $st->type == 'converted' && ($row->client_id != null || $row->client_id != '')){
                        return $action='<select disabled class="form-control statusChange" name="statusChange">' . '<option ' . $selected . ' value="' . $st->id . '">' . $st->type . '</option>' . '</select>';
                    } else if ($st->type != 'converted' ) {
                        if($st->type=='in process'||$st->type=='pending')
                            $statusLi .= '<option ' . $selected . ' value="' . $st->id . '">' . mb_convert_case(__('app.'.$st->type),MB_CASE_TITLE,"UTF-8") . '</option>';
                        else
                        $statusLi .= '<option ' . $selected . ' value="' . $st->id . '">' . mb_convert_case($st->type,MB_CASE_TITLE,"UTF-8") . '</option>';
                    }
                }
            ...
            ->editColumn('client_name', function ($row) {
                if ($row->client_id != null && $row->client_id != '') {
                    $label = '<label class="label label-success">' . ucwords(__('app.menu.clients')) . '</label>';
                } else {
                    $label = '<label class="label label-info">' . ucwords(__('app.menu.lead')) . '</label>';
                }
    ...
    public function query(User $model)
    {
        ...
        $lead = Lead::select('leads.id', 'leads.client_id', 'leads.next_follow_up',
            'client_name', 'company_name', 'lead_status.type as statusName', 'status_id',
            'leads.created_at', 'lead_sources.type as source', 'users.name as agent_name',
            'users.image',
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->buttons(
                Button::make(['extend' => 'export', 'buttons' => ['excel', 'csv'], 'text' => '<i class="fa fa-download"></i> ' . ucwords(trans('app.exportExcel')) . '&nbsp;<span class="caret"></span>'])
            );
    }
...
//            __('modules.lead.leadAgent') => ['data' => 'agent_name', 'name' => 'agent_name'],
