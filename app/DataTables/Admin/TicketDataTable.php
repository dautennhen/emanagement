...
class TicketDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('others', function ($row) {
                ...
                if ($row->status == 'open') {
                    $others .= '<li>' . __('app.status') . ': <label class="label label-danger">' . ucwords(__("app.$row->status")) . '</label></li>';
                } elseif ($row->status == 'pending') {
                    $others .= '<li>' . __('app.status') . ': <label class="label label-warning">' . ucwords(__("app.$row->status")) . '</label></li>';
                } elseif ($row->status == 'resolved') {
                    $others .= '<li>' . __('app.status') . ': <label class="label label-info">' . ucwords(__("app.$row->status")) . '</label></li>';
                } elseif ($row->status == 'closed') {
                    $others .= '<li>' . __('app.status') . ': <label class="label label-success">' . ucwords(__("app.$row->status")) . '</label></li>';
                }
                $others .= '<li>' . __('modules.tasks.priority') . ': ' . ucwords(__("app.$row->priority")) . '</li>
                </ul>';
                return $others;
            })
    ...
    public function query(Ticket $model)
    {
        ...
        $request->startDate = Carbon::parse($request->startDate)->timezone('UTC')->format('Y-m-d');
        $request->endDate = Carbon::parse($request->endDate)->timezone('UTC')->format('Y-m-d');
