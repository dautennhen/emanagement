...
class ProposalController extends AdminBaseController
{
    ...
    {
        $lead = Proposal::select('proposals.id', 'leads.company_name', 'total', 'valid_till', 'proposals.status', 'currencies.currency_symbol')
        ...
        return DataTables::of($lead)
            ->addColumn('action', function ($row) {
                ...
                if ($row->status == 'waiting') {
                    $convert = '<li><a href="' . route("admin.proposals.convert-proposal", $row->id) . '" ><i class="fa fa-file"></i> '. mb_convert_case(__('modules.invoices.ConvertInvoice'), MB_CASE_TITLE, "UTF-8") .' </a></li>';
                }
                return '<div class="btn-group m-r-10">
                <button aria-expanded="false" data-toggle="dropdown" class="btn btn-info btn-outline  dropdown-toggle waves-effect waves-light" type="button">' . mb_convert_case(__('modules.proposal.action'), MB_CASE_TITLE, "UTF-8") . ' <span class="caret"></span></button>
                <ul role="menu" class="dropdown-menu">
                  <li><a href="' . route("admin.proposals.download", $row->id) . '" ><i class="fa fa-download"></i> ' . ucwords(__('modules.proposal.download')) . '</a></li>
                  <li><a href="' . route("admin.proposals.edit", $row->id) . '" ><i class="fa fa-pencil"></i> ' . ucwords(__('modules.proposal.edit')) . ' </a></li>
                  ' . $convert . '
            ...
            ->editColumn('status', function ($row) {
                if ($row->status == 'waiting') {
                    return '<label class="label label-warning">' . mb_convert_case(__('modules.proposal.'.$row->status), MB_CASE_UPPER, "UTF-8") . '</label>';
                }
                if ($row->status == 'declined') {
                    return '<label class="label label-danger">' . mb_convert_case(__('modules.proposal.'.$row->status), MB_CASE_UPPER, "UTF-8") . '</label>';
                } else {
                    return '<label class="label label-success">' . mb_convert_case(__('modules.proposal.'.$row->status), MB_CASE_UPPER, "UTF-8") . '</label>';
                }
            })
