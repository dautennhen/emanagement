        ...
        return DataTables::of($leaves)
            ...
            ->addColumn('action', function ($row) {
                ...
                $action .= '<a href="javascript:;" onclick="getEventDetail(' . $row->id . ')" class="btn btn-info btn-circle"
                      data-toggle="tooltip" data-original-title="'.mb_convert_case(__("app.view"),MB_CASE_TITLE,"UTF-8").'"><i class="fa fa-search" aria-hidden="true"></i></a>';

                if ($row->status == 'pending') {
                    $action .= '  <a href="javascript:;" class="btn btn-danger btn-circle sa-params"
                      data-toggle="tooltip" data-user-id="' . $row->id . '" data-original-title="'.mb_convert_case(__("app.delete"),MB_CASE_TITLE,"UTF-8").'"><i class="fa fa-times" aria-hidden="true"></i></a>';
                }
            ...
            ->editColumn('status', function ($row) {
                if ($row->status == 'approved') {
                    return '<label class="label label-success">' . mb_convert_case(__('app.'.$row->status),MB_CASE_TITLE,"UTF-8") . '</label>';
                } elseif ($row->status == 'pending') {
                    return '<label class="label label-warning">' . mb_convert_case(__('app.'.$row->status),MB_CASE_TITLE,"UTF-8") . '</label>';
                } else {
                    return '<label class="label label-danger">' . mb_convert_case(__('app.'.$row->status),MB_CASE_TITLE,"UTF-8") . '</label>';
                }
            })
