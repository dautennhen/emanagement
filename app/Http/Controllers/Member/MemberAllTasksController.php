...
class MemberAllTasksController extends MemberBaseController
{
    ...
    public function data(Request $request, $hideCompleted = null, $projectId = null)
    {
        ...
        return DataTables::of($tasks)
            ...
            ->addColumn('action', function ($row) {
                $action = '';
                if ($this->user->cans('edit_tasks') || ($this->global->task_self == 'yes' && $this->user->id == $row->created_by)) {
                    $action .= '<a href="' . route('member.all-tasks.edit', $row->id) . '" class="btn btn-info btn-circle"
                      data-toggle="tooltip" data-original-title="'.ucwords(__('app.edit')).'"><i class="fa fa-pencil" aria-hidden="true"></i></a>';
                }

                if ($this->user->cans('delete_tasks') || ($this->global->task_self == 'yes' && $this->user->id == $row->created_by)) {
                    $recurringTaskCount = Task::where('recurring_task_id', $row->id)->count();
                    $recurringTask = $recurringTaskCount > 0 ? 'yes' : 'no';

                    $action .= '&nbsp;&nbsp;<a href="javascript:;" class="btn btn-danger btn-circle sa-params"
                      data-toggle="tooltip" data-task-id="' . $row->id . '" data-recurring="' . $recurringTask . '" data-original-title="'.ucwords(__('app.delete')).'"><i class="fa fa-times" aria-hidden="true"></i></a>';
                }
                return $action;
            })
            ...
            ->editColumn('name', function ($row) {
                ...
                foreach ($row->users as $member) {
                    ...
                    if ($this->user->cans('view_employees')) {
                        $members .= '<a href="' . route('member.employees.show', [$member->id]) . '">';
                    } elseif ($member->id==$this->user->id){
                        $members .= '<a href="' . route('member.profile.index') . '">';
                    } else{
                        $members .= '<a>';
                    }
            ...
            ->editColumn('board_column', function ($row) use ($taskBoardColumns) {
                ...
                if($row->board_column=='Complete'||$row->board_column=='Completed'||$row->board_column=='Incomplete')
                    $status .= '<button aria-expanded="true" data-toggle="dropdown" class="btn dropdown-toggle waves-effect waves-light btn-xs"  style="border-color: ' . $row->label_color . '; color: ' . $row->label_color . '" type="button">' . ucwords(__("app.$row->board_column")) . '</button>';
                else
                    $status .= '<button aria-expanded="true" data-toggle="dropdown" class="btn dropdown-toggle waves-effect waves-light btn-xs"  style="border-color: ' . $row->label_color . '; color: ' . $row->label_color . '" type="button">' . ucwords($row->board_column) . '</button>';
                $status .= '<ul role="menu" class="dropdown-menu pull-right">';
                foreach ($taskBoardColumns as $key => $value) {
                    if($value->column_name=='Complete'||$value->column_name=='Completed'||$value->column_name=='Incomplete')
                        $status .= '<li><a href="javascript:;" data-task-id="' . $row->id . '" class="change-status" data-status="' . $value->slug . '">' . ucwords(__("app.$value->column_name")) . '  <span style="width: 15px; height: 15px; border-color: ' . $value->label_color . '; background: ' . $value->label_color . '"
                        class="btn btn-warning btn-small btn-circle">&nbsp;</span></a></li>';
                    else
                        $status .= '<li><a href="javascript:;" data-task-id="' . $row->id . '" class="change-status" data-status="' . $value->slug . '">' . ucwords($value->column_name) . '  <span style="width: 15px; height: 15px; border-color: ' . $value->label_color . '; background: ' . $value->label_color . '"
                        class="btn btn-warning btn-small btn-circle">&nbsp;</span></a></li>';
                }
                $status .= '</ul>';
                $status .= '</div>';
    ...
    {
        $this->task = Task::with('board_column', 'subtasks', 'project', 'users', 'files', 'comments', 'label')->findOrFail($id);
