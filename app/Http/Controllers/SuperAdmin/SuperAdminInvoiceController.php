...
class SuperAdminInvoiceController extends SuperAdminBaseController
{
    ...
    public function index()
    {
        ...
        $vnpayInvoices = DB::table("vnpay_invoices")
            ->whereNotNull('vnpay_invoices.pay_date')->count();
        ...
        $this->totalInvoices = ($stripeInvoices + $PaypalInvoices + $razorpayInvoice + $paystackInvoices + $vnpayInvoices + $mollieInvoices + $authorizeInvoices);
    ...
    public function data()
    {
        ...
        $vnpay = DB::table("vnpay_invoices")
            ->join('packages', 'packages.id', 'vnpay_invoices.package_id')
            ->join('companies', 'companies.id', 'vnpay_invoices.company_id')
            ->selectRaw('vnpay_invoices.id ,"" as invoice_id , companies.company_name as company,
             packages.name as name, vnpay_invoices.transaction_id, "VNpay" as method,vnpay_invoices.amount, vnpay_invoices.pay_date as paid_on ,
             vnpay_invoices.next_pay_date,"" as offline_method_id')
            ->whereNotNull('vnpay_invoices.pay_date');
        ...
        return Datatables::of($offline)
            ...
            ->addColumn('action', function ($row) {
                ...
                if($row->method == 'VNpay') {
                    return '<a href="'.route('super-admin.vnpay.invoice-download', $row->id).'" class="btn btn-primary btn-circle waves-effect" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a>';
                }
    ...
    public function vnpayInvoiceDownload($id)
    {
        $this->invoice = VNpayInvoice::with(['company','package'])->findOrFail($id);
        $pdf = app('dompdf.wrapper');
        $this->company = $this->invoice->company;
        $pdf->loadView('vnpay-invoice.invoice-1', $this->data);
        $filename = $this->invoice->pay_date->format("dS M Y").'-'.$this->invoice->next_pay_date->format("dS M Y");
        return $pdf->download($filename . '.pdf');
    }
